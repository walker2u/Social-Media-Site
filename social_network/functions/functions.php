	...
	if(isset($_POST['sub'])){
		...
		if(strlen($content) > 250){
			...
			if(strlen($upload_image) >= 1 && strlen($content) >= 1){
				...
				if($run){
					echo "<script>alert('Your Post updated a moment ago with an image and text!')</script>";
					echo "<script>window.open('home.php', '_self')</script>";
...
function get_posts(){
	...
	function single_post(){
		...
		if(isset($_GET['post_id'])){
			...
			$user_comm = $_SESSION['user_email'];
			$get_comm = "select* from users where user_email = '$user_comm'";
			$run_comm = mysqli_query($con,$get_comm);
			$row_comm = mysqli_fetch_array($run_comm);
			
			$user_comm_id = $row_comm['user_id'];
			$user_comm_name = $row_comm['user_name'];
					...
					<div class='panel panel-info'>
						<div class='panel-body'>
							<form action='' method='post' class='form-inline'>
								<textarea placeholder='Write your comment here..' class='pb-cmnt-textarea' name='comment'>
		...
		if(isset($_POST['reply'])){
			...
			if($comment == ""){
				...
				$insert = "insert into comments (post_id, user_id, comment, comment_author, date) values('$post_id','$user_id','$comment','$user_comm_name',NOW())";
				$run = mysqli_query($con, $insert);
